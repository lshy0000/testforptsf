package com.wheatek.smartdevice.wheelview.view;
...
import com.wheatek.smartdevice.wheelview.interfaces.IPickerViewData;
import com.wheatek.smartdevice.wheelview.listener.LoopViewGestureListener;
import com.wheatek.smartdevice.wheelview.listener.OnItemSelectedListener;
import com.wheatek.smartdevice.wheelview.timer.InertiaTimerTask;
import com.wheatek.smartdevice.wheelview.timer.MessageHandler;
import com.wheatek.smartdevice.wheelview.timer.SmoothScrollTimerTask;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (dividerType == DividerType.WRAP) {//横线长度仅包裹内容
            ...
            LinearGradient lg = new LinearGradient((endX - (endX - startX) / 2), firstLineY, startX, firstLineY,
                    getResources().getColor(R.color.line_pmsf, null),
                    getResources().getColor(android.R.color.transparent, null), Shader.TileMode.MIRROR);
...
//                    float Y = maxTextHeight - CENTER_CONTENT_OFFSET;//因为圆弧角换算的向下取值，导致角度稍微有点偏差，加上画笔的基线会偏上，因此需要偏移量修正一下
                    float Y = maxTextHeight / 2;
                    Y = Y + getBaseline(paintCenterText)+1;
                    canvas.drawText(contentText, drawCenterContentStart, Y, paintCenterText);
                    //设置选中项
                    selectedItem = preCurrentIndex - (itemsVisible / 2 - counter);
//                    float iy = 0;
//                    canvas.drawText(contentText, drawCenterContentStart, iy, paintCenterText);
//                    selectedItem = preCurrentIndex - (itemsVisible / 2 - counter);
                } else {
                    // 其他条目
                    canvas.save();
